...
const handleCompare = (pokemon) => {
    ...
    if (isSelected) {
        // If selected, remove it from the comparison list
        selectedPokemon = selectedPokemon.filter((p) => p.id !== pokemon.id);
    } else {
        // Otherwise, add it to the comparison list
...
const updateComparison = () => {
    ...
    // Clear the current comparison display
    ...
    // Create a table to display comparison stats
    let comparisonHTML = `<table><tr><th>Stat</th>`;
    selectedPokemon.forEach((pokemon) => {
        comparisonHTML += `<th>${pokemon.name[0].toUpperCase() + pokemon.name.slice(1)}</th>`;
    });
    comparisonHTML += `</tr>`;

    // Assuming we are comparing the first three stats (HP, Attack, Defense)
